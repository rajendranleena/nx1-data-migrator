...
```bash
# Scan a directory
python spark_migration_scanner.py /path/to/your/project

# Generate HTML report
python spark_migration_scanner.py /path/to/project -f html -o report.html

# Generate JSON report (for CI/CD integration)
python spark_migration_scanner.py /path/to/project -f json -o report.json

# Filter by severity
python spark_migration_scanner.py /path/to/project -s high

# Filter by category
python spark_migration_scanner.py /path/to/project -c "Spark API Changes" -c "HDFS to S3 Migration"
```
...
Generates a styled, interactive HTML page with collapsible sections, color-coded severities, and search functionality.
  ...
  migration-scan:
    ...
    steps:
      ...
      - name: Run Migration Scanner
        run: |
          python spark_migration_scanner.py . -f json -o migration-report.json -s high
...
pipeline {
    ...
    stages {
        stage('Migration Scan') {
            steps {
                sh 'python spark_migration_scanner.py . -f json -o migration-report.json'
            }
        }
        stage('Check Results') {
            steps {
...
- [ ] **Remove Optimizer Properties**
  ...
  - [ ] Remove SET hive.cbo.* (use spark.sql.cbo.*)
...
2. **Pattern Matching**: May produce false positives for commented code or strings
